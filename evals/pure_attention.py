...
def evaluate(args):
    ...
    # DataLoader
    loader = DataLoader(
        SCNDataset(args.data_folder, args.data_name, 'TEST'),
    ...
    with open('./evaluation/pure_attention_beam_{}_references.json'.format(args.beam_size), 'w') as f:
        json.dump(references, f)
        f.close()

    with open('./evaluation/pure_attention_beam_{}_hypotheses.json'.format(args.beam_size), 'w') as f:
        json.dump(hypotheses, f)
        f.close()

    scores = n.compute_metrics(ref_list=references, hyp_list=hypotheses)

    with open('./evaluation/pure_attention_beam_{}_scores.json'.format(args.beam_size), 'w') as f:
        json.dump(scores, f)
        f.close()
