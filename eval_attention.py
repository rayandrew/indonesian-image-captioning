...
# model checkpoint
checkpoint = './BEST_checkpoint_attention_flickr10k_5_cap_per_img_5_min_word_freq.pth.tar'
...
def evaluate(beam_size):
    ...
    with open('./evaluation/attention_beam_{}_references.json'.format(beam_size), 'w') as f:
        json.dump(references, f)
        f.close()

    with open('./evaluation/attention_beam_{}_hypotheses.json'.format(beam_size), 'w') as f:
        json.dump(hypotheses, f)
        f.close()

    scores = n.compute_metrics(ref_list=references, hyp_list=hypotheses)

    with open('./evaluation/attention_beam_{}_scores.json'.format(beam_size), 'w') as f:
        json.dump(scores, f)
        f.close()
...
if __name__ == '__main__':
    beam_size = 5
    print("\nScores of Attention Model @ beam size of {} is {}.".format(
